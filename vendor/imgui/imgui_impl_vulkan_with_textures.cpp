...
static inline float MyAbs(float val)
{
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    scissor.extent.width = (uint32_t)(MyAbs(clip_rect.z - clip_rect.x));
                    scissor.extent.height = (uint32_t)(MyAbs(clip_rect.w - clip_rect.y));
...
{
    vkResetDescriptorPool(g_VulkanInitInfo.Device, g_VulkanInitInfo.PerFrameDescriptorPool, 0);
}
...
void ImGui_SetPerFrameDescriptorPool(VkDescriptorPool pool)
{
    g_VulkanInitInfo.PerFrameDescriptorPool = pool;
}
